...
def check(sun, cloud, human): #if sun.vec intersec the cloud
    line = LineString([human.point(), sun.vec()])
...
def plotting(sun, cloud_list, human, t,number_of_clouds):
    xs = [human.x, (sun.vec()[0])]
    ys = [human.y, (sun.vec()[1])]
    zs = [human.z, (sun.vec()[2])]
    fig = plt.figure()
    ...
    for i in range(number_of_clouds):
        ...
        y = cloud_list[i].sety()
        z = cloud_list[i].setz()
        ax.plot_trisurf(x, y, z, linewidth=0.2,color='grey')
